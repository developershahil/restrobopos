    ...
    <div className="space-y-6">
        ...
        <div className="relative">
          ...
          {showTypeDropdown && (
            <>
              ...
              <div className="absolute right-0 top-full mt-2 w-56 bg-card border border-border rounded-md shadow-2xl z-50 py-2 animate-in fade-in slide-in-from-top-2">
            ...
            <tbody>
              {offers.map(offer => (
                <tr key={offer.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                  <td className="px-5 py-2 text-sm font-mono font-semibold text-foreground">{offer.code}</td>
                  <td className="px-5 py-2 text-sm text-center text-muted-foreground font-medium">{offer.type}</td>
                  <td className="px-5 py-2 text-sm text-center font-medium text-foreground">{offer.title}</td>
                  <td className="px-5 py-2 text-sm text-center text-muted-foreground max-w-xs truncate">{offer.description}</td>
                  ...
                  <td className="px-5 py-2">
                    <div className="flex items-center justify-center gap-3">
                      ...
                      >
                        <div className={`w-4 h-4 bg-white rounded-full absolute top-1 shadow-sm transition-all ${offer.active ? 'left-8' : 'left-1'}`} />
                      </button>
                      <button
      ...
      {linkModal.open && linkModal.offer && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          {/* Modal */}
          <div className="relative bg-card border border-border rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[80vh] flex flex-col">
            {/* Header */}
            ...
            {/* Search + Select All */}
            <div className="px-3 py-2 border-b border-border space-y-3 shrink-0">
              <div className="relative">
                ...
                .map(outlet => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-2 rounded-md mb-1 transition-all ${
                        isLinked
            ...
            {/* Footer */}
            <div className="flex items-center justify-end gap-3 px-3 py-2 border-t border-border shrink-0">
              <button
                onClick={() => setLinkModal({ open: false, offer: null })}
