...
function Checkbox({ checked, partial, onClick, size = 'md' }) {
  const sz = size === 'sm' ? 'w-4 h-4' : 'w-4 h-4';
...
export default function SelectCategoriesModal({ open, onClose, onDone, selectedCategories = [], mode = 'categories' }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-card border border-border rounded-lg shadow-2xl w-[95vw] max-w-6xl h-[90vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="relative">
            ...
            {outletDropdownOpen && (
              <>
                ...
                <div className="absolute left-0 right-0 top-full mt-1 bg-card border border-border rounded-md shadow-xl z-20 py-1 max-h-48 overflow-y-auto">
                  ...
                  {MOCK_OUTLETS.filter(o => !outletSearch || o.name.toLowerCase().includes(outletSearch.toLowerCase())).length === 0 && (
                    <div className="px-3 py-2 text-sm text-muted-foreground text-center">No outlets found</div>
                  )}
                </div>
              </>
            )}
        ...
        <div className="flex items-center justify-between px-3 py-2 border-t border-border shrink-0">
          ...
          <button onClick={handleDone} className="px-4 py-2.5 bg-primary text-primary-foreground rounded-lg text-sm font-bold hover:bg-primary/90 transition-colors shadow-sm">
            Done ({totalSelected})
          </button>
        </div>
      </div>
