...
export default function Profile() {
  ...
  return (
    <div className="flex flex-col h-full bg-background overflow-hidden">
      {/* Header */}
      <div className="p-4 md:p-4 border-b border-border bg-card shrink-0 flex items-center gap-4">
        <button 
          onClick={() => navigate(-1)} 
          className="p-2.5 border border-border bg-background rounded-md hover:bg-muted transition-colors shadow-sm shrink-0"
          title="Go Back"
        >
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
        ...
        <div className="w-full md:w-64 border-b md:border-b-0 md:border-r border-border bg-background p-4 flex flex-row md:flex-col gap-2 shrink-0 overflow-x-auto md:overflow-y-auto scrollbar-hide">
          <button 
            ...
            className={`flex items-center gap-3 px-3 py-2 rounded-md font-bold text-sm transition-all whitespace-nowrap ${activeTab === 'personal' ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}
          >
            <User className="w-4 h-4" /> Personal Info
          </button>
          <button 
            onClick={() => setActiveTab('security')}
            className={`flex items-center gap-3 px-3 py-2 rounded-md font-bold text-sm transition-all whitespace-nowrap ${activeTab === 'security' ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}
          >
            <Shield className="w-4 h-4" /> Security
          </button>
          <button 
            onClick={() => setActiveTab('notifications')}
            className={`flex items-center gap-3 px-3 py-2 rounded-md font-bold text-sm transition-all whitespace-nowrap ${activeTab === 'notifications' ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}
          >
            <Bell className="w-4 h-4" /> Notifications
          </button>
          <button 
            onClick={() => setActiveTab('sessions')}
            className={`flex items-center gap-3 px-3 py-2 rounded-md font-bold text-sm transition-all whitespace-nowrap ${activeTab === 'sessions' ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}
          >
        ...
        {/* Main Content Area */}
        <div className="flex-1 overflow-y-auto p-4 md:p-5 lg:p-10 bg-card">
          <div className="max-w-3xl">
            ...
            {activeTab === 'personal' && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="flex justify-end pt-4 border-t border-border">
                  <button className="px-4 py-2.5 bg-primary text-primary-foreground font-bold rounded-lg shadow-md hover:bg-primary/90 transition-colors">
                    Save Changes
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div>
                  ...
                  <div className="space-y-4">
                    ...
                    <button className="mt-4 px-4 py-2.5 bg-background border border-border text-foreground font-bold rounded-lg shadow-sm hover:bg-muted transition-colors">
                      Update Password
                    </button>
                  </div>
                </div>
                ...
                <div className="pt-8 border-t border-border">
                  ...
                  <div className="flex items-start justify-between bg-muted/30 p-4 rounded-md border border-border">
                    <div className="pr-4">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="w-14 h-7 bg-green-500 rounded-full relative cursor-pointer">
                      <div className="w-4 h-4 bg-white rounded-full absolute left-8 top-1 shadow-sm"></div>
                    </div>
                  </div>
            ...
            {activeTab === 'sessions' && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="bg-primary/5 border border-primary/20 rounded-md p-4 flex items-start justify-between">
