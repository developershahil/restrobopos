...
export default function Stores() {
  ...
  return (
    <div className="flex flex-col h-full bg-background overflow-hidden">
      {/* Header */}
      <div className="p-4 border-b border-border bg-card shrink-0 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold tracking-tight flex items-center gap-2"><Store className="w-6 h-6 text-primary" /> Store Management</h1>
      ...
      <div className="flex-1 overflow-y-auto p-4 bg-muted/10">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          {filteredStores.map(store => (
            <div key={store.id} className="bg-card border border-border rounded-md shadow-sm overflow-hidden flex flex-col transition-all hover:shadow-md">
              ...
              <div className="p-5 border-b border-border bg-muted/30 flex justify-between items-start">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button className="p-1 hover:bg-muted rounded-md text-muted-foreground"><MoreVertical className="w-4 h-4" /></button>
                </div>
              </div>

              {/* Card Body (Details & Channels) */}
              <div className="p-5 flex-1 flex flex-col gap-4 opacity-100 transition-opacity" style={{ opacity: store.masterStatus ? 1 : 0.6 }}>
                
                {/* Metrics */}
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-background border border-border rounded-lg p-3">
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Today's Revenue</p>
                    <p className="text-lg font-bold mt-0.5">{store.metrics.revenue}</p>
                  </div>
                  <div className="bg-background border border-border rounded-lg p-3">
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Total Orders</p>
                    <p className="text-lg font-bold mt-0.5">{store.metrics.orders}</p>
                  </div>
                </div>
