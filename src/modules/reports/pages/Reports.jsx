...
export default function Reports() {
  ...
  return (
    <div className="flex flex-col h-full bg-background overflow-hidden relative">
      ...
      {showExportToast && (
        <div className="absolute top-4 right-6 bg-primary text-primary-foreground px-3 py-2 rounded-md shadow-2xl flex items-start gap-3 z-50 animate-in slide-in-from-top-5">
      ...
      {/* Header */}
      <div className="p-4 border-b border-border bg-card shrink-0 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {exportingFormat === 'csv' ? <span className="animate-pulse">Preparing...</span> : <><FileSpreadsheet className="w-4 h-4 text-green-600" /> Export CSV</>}
          </button>
          <button 
            ...
            className="px-4 py-2 bg-background border border-border text-foreground font-bold rounded-lg shadow-sm hover:bg-muted transition-colors flex items-center gap-2"
          >
            {exportingFormat === 'pdf' ? <span className="animate-pulse">Preparing...</span> : <><FileText className="w-4 h-4 text-red-500" /> Export PDF</>}
          </button>
        </div>
      </div>

      {/* Universal Filter Bar */}
      <div className="px-3 py-2 bg-muted/30 border-b border-border shrink-0 flex flex-wrap items-center gap-4">
        ...
        <div className="w-64 border-r border-border bg-card p-4 flex flex-col gap-1.5 shrink-0 overflow-y-auto">
          {reportTabs.map(tab => (
            <button 
              ...
              className={`flex items-center gap-3 px-3 py-2 rounded-md font-bold text-sm transition-all ${activeTab === tab.id ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}
            >
        ...
        <div className="flex-1 overflow-y-auto bg-background p-4">
          ...
          {activeTab === 'sales' && (
            <div className="space-y-6 animate-in fade-in duration-300">
              <h2 className="text-lg font-bold">Sales & Revenue Breakdown</h2>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-border">
                      <tr>
                        <td className="px-3 py-2 font-medium">Oct 24, 2026</td>
                        <td className="px-3 py-2">142</td>
                        <td className="px-3 py-2">$4,200.00</td>
                        <td className="px-3 py-2 text-red-500">-$120.00</td>
                        <td className="px-3 py-2 font-bold">$4,080.00</td>
                        <td className="px-3 py-2">$408.00</td>
                      </tr>
                      <tr>
                        <td className="px-3 py-2 font-medium">Oct 23, 2026</td>
                        <td className="px-3 py-2">128</td>
                        <td className="px-3 py-2">$3,850.00</td>
                        <td className="px-3 py-2 text-red-500">-$95.00</td>
                        <td className="px-3 py-2 font-bold">$3,755.00</td>
                        <td className="px-3 py-2">$375.50</td>
                      </tr>
                      <tr>
                        <td className="px-3 py-2 font-medium">Oct 22, 2026</td>
                        <td className="px-3 py-2">156</td>
                        <td className="px-3 py-2">$4,800.00</td>
                        <td className="px-3 py-2 text-red-500">-$210.00</td>
                        <td className="px-3 py-2 font-bold">$4,590.00</td>
                        <td className="px-3 py-2">$459.00</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
          ...
          {activeTab === 'items' && (
            <div className="space-y-6 animate-in fade-in duration-300">
              <h2 className="text-lg font-bold">Item Performance (Menu Intelligence)</h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="bg-card border border-border rounded-md p-5 shadow-sm">
                  <h3 className="font-bold flex items-center gap-2 mb-4"><TrendingUp className="w-4 h-4 text-green-500"/> Top 5 Bestsellers</h3>
                ...
                <div className="bg-card border border-border rounded-md p-5 shadow-sm">
                  <h3 className="font-bold flex items-center gap-2 mb-4"><AlertCircle className="w-4 h-4 text-red-500"/> The Laggards (Bottom 3)</h3>
          ...
          {['orders', 'customers', 'inventory', 'profit', 'offers'].includes(activeTab) && (
            <div className="flex flex-col items-center justify-center h-full text-muted-foreground animate-in fade-in duration-300">
              ...
              <h2 className="text-lg font-bold text-foreground">Detailed {reportTabs.find(t=>t.id===activeTab)?.name} Report</h2>
