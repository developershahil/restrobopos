  ...
  return (
    <div className="flex flex-col h-full bg-muted/30 overflow-hidden relative">
      ...
      <div className="p-4 border-b border-border bg-card flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0">
        <div>
          <h1 className="text-lg font-bold text-foreground">Third Party Menus</h1>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        <section className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="bg-gradient-to-br from-card to-muted/50 border border-primary/20 rounded-lg p-5 shadow-sm relative overflow-hidden group">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-5 relative z-10">
              ...
              <div className="w-full md:w-auto">
                <button 
                  ...
                  className={`w-full md:w-auto px-8 py-2 rounded-lg text-base font-black flex items-center justify-center gap-3 transition-all shadow-xl ${showFullMenu ? 'bg-foreground text-background' : 'bg-primary text-primary-foreground shadow-primary/20 hover:opacity-90'}`}
                >
          ...
          {showFullMenu && (
            <div className="bg-card border border-border rounded-lg overflow-hidden shadow-sm animate-in fade-in slide-in-from-top-4 duration-300">
                ...
                {categories.map(cat => (
                  <div key={cat.id} className="space-y-4">
                    <div className="flex items-center gap-3 border-l-4 border-primary pl-4">
                      <h4 className="text-lg font-bold text-foreground">{cat.name}</h4>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pl-5">
                      {items.filter(i => i.categoryId === cat.id).map(item => (
                        <div key={item.id} className="p-3 bg-muted/20 border border-border rounded-md flex justify-between items-center group hover:border-primary/30 transition-colors">
                          ...
                          <span className="font-black text-[13px] text-foreground">${item.price}</span>
                        </div>
                      ))}
                    </div>
                  </div>
