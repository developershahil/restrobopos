...
export default function MenuPushTab() {
  ...
  return (
    <div className="flex h-full bg-background overflow-hidden flex-col md:flex-row">
      ...
      <div className="w-full md:w-1/2 p-4 md:p-4 md:border-r border-b md:border-b-0 border-border overflow-y-auto custom-scrollbar flex flex-col">
        <div className="mb-6">
        ...
        <div className="mb-8">
          <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground mb-3 flex items-center gap-2">
            <span className="w-4 h-4 rounded-full bg-muted flex items-center justify-center text-[10px] text-foreground">1</span> 
            Target Menu
          </h3>
          <select
            value={selectedPushMenuId}
            onChange={(e) => setSelectedPushMenuId(e.target.value)}
            className="w-full bg-background border border-border rounded-md px-3 py-2 text-sm font-bold outline-none focus:border-primary cursor-pointer transition-colors shadow-sm"
          >
            {menus.map(m => (
        ...
        <div className="mb-8">
          <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground mb-3 flex items-center gap-2">
            <span className="w-4 h-4 rounded-full bg-muted flex items-center justify-center text-[10px] text-foreground">2</span> 
            Select Platforms
          </h3>
        ...
        <div className="mb-8">
          <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground mb-3 flex items-center gap-2">
            <span className="w-4 h-4 rounded-full bg-muted flex items-center justify-center text-[10px] text-foreground">3</span> 
            Deployment Timing
          </h3>
          
          <div className="flex gap-4 mb-4">
            <label className={`flex-1 flex flex-col p-4 rounded-md border-2 cursor-pointer transition-all ${scheduleType === 'now' ? 'border-primary bg-primary/5' : 'border-border bg-card'}`}>
            ...
            <label className={`flex-1 flex flex-col p-4 rounded-md border-2 cursor-pointer transition-all ${scheduleType === 'later' ? 'border-primary bg-primary/5' : 'border-border bg-card'}`}>
              <div className="flex items-center gap-2 mb-1">
          ...
          {scheduleType === 'later' && (
            <div className="flex gap-4 p-4 bg-muted/30 border border-border rounded-md animate-in slide-in-from-top-2 duration-200">
        ...
        <div className="mb-8 flex-1">
          <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground mb-3 flex items-center gap-2">
            <span className="w-4 h-4 rounded-full bg-muted flex items-center justify-center text-[10px] text-foreground">4</span> 
        ...
        <div className="pt-4 border-t border-border mt-auto">
          <button 
            ...
            className={`w-full py-3 rounded-md text-sm font-bold flex items-center justify-center gap-2 transition-all shadow-md ${
              pushSuccess 
          ...
          >
            {pushSuccess ? (
              ...
              <><Send className="w-4 h-4" /> Deploy Menu Now</>
            ) : (
      ...
      <div className="w-full md:w-1/2 flex flex-col bg-muted/10 md:border-l border-border">
        ...
        {/* Pending Queue */}
        <div className="flex-1 p-4 md:p-4 border-b border-border overflow-y-auto custom-scrollbar">
          <h3 className="text-sm font-bold uppercase tracking-wider text-foreground mb-4">Scheduled Pushes</h3>
          
          {scheduledPushes.length === 0 ? (
            <div className="border border-dashed border-border rounded-md py-8 text-center text-muted-foreground bg-card">
            ...
            <div className="space-y-3">
              {scheduledPushes.map(push => (
                <div key={push.id} className="bg-card border border-border rounded-md p-4 shadow-sm flex items-start gap-4">
        ...
        {/* History */}
        <div className="flex-1 p-4 md:p-4 overflow-y-auto custom-scrollbar">
            ...
            {pushHistory.map((hist, idx) => (
              <div key={hist.id} className="flex gap-4 relative">
                ...
                {idx !== pushHistory.length - 1 && (
                  <div className="absolute left-3 top-5 bottom-[-16px] w-px bg-border z-0" />
                )}
