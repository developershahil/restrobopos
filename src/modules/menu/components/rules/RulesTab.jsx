...
export default function RulesTab() {
  return (
    <div className="flex flex-col h-full bg-background overflow-hidden relative">
      <div className="p-4 border-b border-border bg-card shrink-0 flex justify-between items-center">
        <div>
          <h2 className="text-lg font-bold text-foreground">Menu Rules Engine</h2>
      ...
      <div className="flex-1 overflow-auto p-4">
        <div className="max-w-4xl mx-auto space-y-4">
          {mockRules.map(rule => (
            <div key={rule.id} className={`bg-card border rounded-md p-5 shadow-sm transition-colors ${rule.status ? 'border-primary/30' : 'border-border opacity-60'}`}>
              <div className="flex items-start justify-between">
                <div className="flex gap-4">
                  <div className={`p-3 rounded-md shrink-0 ${rule.status ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>
