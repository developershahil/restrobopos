...
export function ModalWrapper({ title, onClose, children, footer, maxWidth = 'max-w-sm' }) {
  ...
  return (
    <div ref={overlayRef} onClick={(e) => { if (e.target === overlayRef.current) onClose(); }}
      ...
      <div className={`bg-card border border-border rounded-md shadow-xl w-full mx-4 flex flex-col ${maxWidth}`}>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="space-y-2 max-h-[30vh] overflow-y-auto pr-2">
            ...
            {items.filter(i => i.name.toLowerCase().includes(searchTerm.toLowerCase())).length === 0 && (
              <div className="text-center py-2 text-xs text-muted-foreground">No items found matching "{searchTerm}"</div>
            )}
          </div>
        </div>
      )}
...
export function ConfirmModal({ title, message, confirmText = 'Delete', onConfirm, onClose }) {
  ...
  return (
    <div ref={overlayRef} onClick={(e) => { if (e.target === overlayRef.current) onClose(); }}
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-card border border-border rounded-md shadow-xl w-full max-w-sm mx-4 flex flex-col animate-in fade-in zoom-in-95">
        <div className="p-5">
