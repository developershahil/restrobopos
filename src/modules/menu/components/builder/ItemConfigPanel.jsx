...
import { getItemTax, totalRate } from '@modules/menu/utils/taxUtils';
...
function DetailsTab({ item }) {
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="mb-4">
        {imageUrl ? (
          <div className="relative h-32 rounded-md overflow-hidden group border border-border">
          ...
          <button onClick={() => setImageUrl('https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=400&q=80')} className="w-full h-24 rounded-md border-2 border-dashed border-border bg-muted/20 hover:bg-muted/50 hover:border-primary/50 transition-colors flex flex-col items-center justify-center gap-1.5 text-muted-foreground group">
      ...
      <div className="sticky bottom-0 pb-2 pt-4 bg-gradient-to-t from-card via-card to-transparent mt-4 -mx-1 px-1">
        <button 
          onClick={save} 
          disabled={!isDirty}
          className={`w-full py-2 rounded-md text-xs font-bold transition-all shadow-md ${isDirty ? 'bg-primary text-primary-foreground hover:bg-primary/90 hover:-translate-y-0.5' : 'bg-muted text-muted-foreground opacity-60 cursor-not-allowed shadow-none'}`}
        >
...
export default function ItemConfigPanel() {
  ...
  if (!item) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-center px-4 bg-muted/20">
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 bg-background w-full">
      {/* Item Header */}
      <div className="px-3 py-2 border-b border-border bg-card flex items-center gap-2.5">
