...
export default function RecipeCostTab() {
  ...
  return (
    <div className="h-full flex flex-col bg-card relative">
      ...
      <div className="flex-1 overflow-auto p-4 sm:p-4">
        <div className="bg-background border border-border rounded-lg shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left whitespace-nowrap min-w-[800px]">
              <thead className="text-xs text-muted-foreground uppercase bg-muted/50 sticky top-0 z-10">
              <tr>
                <th className="px-5 py-2 font-bold">Menu Item</th>
                <th className="px-5 py-2 font-bold">Category</th>
                <th className="px-5 py-2 font-bold text-right">Selling Price</th>
                <th className="px-5 py-2 font-bold text-right">Recipe Cost</th>
                <th className="px-5 py-2 font-bold text-right">Gross Margin</th>
              ...
              {filteredItems.map((item) => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-muted/30">
                    <td className="px-5 py-2 font-bold text-foreground">{item.name}</td>
                    <td className="px-5 py-2 text-xs text-muted-foreground font-medium">{category}</td>
                    <td className="px-5 py-2 font-black text-right text-foreground">₹{sellPrice.toFixed(2)}</td>
                    <td className="px-5 py-2 font-black text-right text-red-600">₹{recipeCost.toFixed(2)}</td>
