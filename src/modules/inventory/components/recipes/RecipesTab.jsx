...
function LevelCard({ color, label, title, icon: Icon, children, defaultOpen = false }) {
  ...
  return (
    <div className={`border ${c.border} rounded-md overflow-hidden mb-5`}>
      <button
        onClick={() => setOpen(!open)}
        className={`w-full flex items-center justify-between px-5 py-2 ${c.bg} transition-colors hover:brightness-95`}
      >
    ...
    <div className="h-full flex flex-col md:flex-row bg-background">
              ...
              <div key={cat.id} className="mb-1">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    className={`flex-1 flex items-center justify-between px-3 py-2.5 rounded-md transition-all text-left ${
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        {selectionMode === 'category' && selectedCategory && (
          <>
            <div className="px-4 sm:px-3 py-2 border-b border-border bg-card shrink-0 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h2 className="text-lg font-bold text-foreground line-clamp-1">{selectedCategory.name}</h2>
          ...
          <>
            <div className="px-4 sm:px-3 py-2 border-b border-border bg-card shrink-0 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h2 className="text-lg font-bold text-foreground line-clamp-1">{selectedItem.name}</h2>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-4">
              ...
              {hasBom(`cat-${itemCategory?.id}`) && (
                <div className="bg-purple-50 border border-purple-200 rounded-md p-3 mb-5 flex items-start gap-3">
              ...
              <LevelCard color="indigo" label="Variant Level" title={`Variant Recipes (${itemVariants.length} variants)`} icon={Layers} defaultOpen={itemVariants.length > 0}>
                {itemVariants.length === 0 ? (
                  <p className="text-sm text-muted-foreground text-center py-2">No variants for this item. Add variants in the Menu Builder first.</p>
                ) : (
                  <>
