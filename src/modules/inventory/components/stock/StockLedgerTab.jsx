    ...
    <div className="h-full flex flex-col bg-background overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
            ...
            <div className="bg-card border border-border rounded-lg overflow-hidden shadow-sm">
              ...
              <div className="divide-y divide-border">
                {ingredients.map((ing) => {
                  ...
                  return (
                    <div key={ing.id} className="px-5 py-2">
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-[13px] text-foreground">{ing.name}</span>
            ...
            <div className="bg-card border border-border rounded-lg overflow-hidden shadow-sm">
              ...
              <div className="divide-y divide-border">
                {transactions.slice(0, 5).map((txn) => {
                  ...
                  const Icon = cfg?.icon;
                  return (
                    <div key={txn.id} className="px-5 py-2 flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-bold text-[13px] text-foreground">{ing?.name || txn.ingredientId}</p>
            ...
            <div className="flex-1 overflow-auto">
              <div className="overflow-x-auto min-h-0">
                <table className="w-full text-sm text-left whitespace-nowrap min-w-[1000px]">
                  ...
                  <tr>
                    <th className="px-5 py-2 font-bold">Txn ID</th>
                    <th className="px-5 py-2 font-bold">Date & Time</th>
                    <th className="px-5 py-2 font-bold">Type</th>
                    <th className="px-5 py-2 font-bold">Ingredient</th>
                    <th className="px-5 py-2 font-bold">Outlet</th>
                    <th className="px-5 py-2 font-bold text-right">Qty</th>
                    <th className="px-5 py-2 font-bold text-right">Balance After</th>
                    <th className="px-5 py-2 font-bold">Note / PO Ref</th>
                  </tr>
                </thead>
          ...
          <div className="p-4 space-y-4">
            {filteredPOs.length === 0 ? (
              ...
              filteredPOs.map((po) => {
                ...
                return (
                  <div key={po.id} className="bg-card border border-border rounded-lg overflow-hidden shadow-sm">
                    <div className="px-5 py-2 flex items-center justify-between border-b border-border bg-muted/20">
        ...
        {activeSection === 'wastage' && (
          <div className="p-4 space-y-4">
            <div className="bg-red-50 border border-red-200 rounded-lg p-5 shadow-sm flex items-center justify-between">
              <div>
                <p className="text-xs font-bold text-red-700 uppercase tracking-wider mb-1">Total Loss Value</p>
                <h3 className="text-xl font-bold text-red-800">₹{totalWastageLoss.toFixed(2)}</h3>
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left whitespace-nowrap min-w-[800px]">
                    ...
                    <tr>
                      ...
                      <th className="px-5 py-2 font-bold">Ingredient</th>
                      <th className="px-5 py-2 font-bold">Reason</th>
                      <th className="px-5 py-2 font-bold">Qty Lost</th>
                      <th className="px-5 py-2 font-bold text-right">Loss Value</th>
                      <th className="px-5 py-2 font-bold">Note</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border">
        ...
        {activeSection === 'daily' && (
          <div className="p-4 flex flex-col h-full space-y-4">
            <div className="flex items-center gap-4 bg-muted/20 p-4 rounded-md border border-border shrink-0">
            ...
            <div className="flex-1 overflow-auto bg-card border border-border rounded-lg shadow-sm">
              <div className="overflow-x-auto min-h-0">
                <table className="w-full text-sm text-left whitespace-nowrap min-w-[900px]">
                  ...
                  <tr>
                    <th className="px-5 py-2 font-bold">Ingredient</th>
                    <th className="px-5 py-2 font-bold text-right bg-muted/30">Opening Balance</th>
                    <th className="px-5 py-2 font-bold text-right text-green-700">Stock In (+)</th>
                    <th className="px-5 py-2 font-bold text-right text-blue-700">Consumed (-)</th>
                    <th className="px-5 py-2 font-bold text-right text-red-700">Wastage (-)</th>
                    <th className="px-5 py-2 font-bold text-right bg-muted/30">Closing Balance</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {dailyReport.map((row) => (
