...
export default function ItemAvailability() {
  ...
  return (
    <div className="flex flex-col h-full bg-background overflow-hidden">
      {/* Header */}
      <div className="p-4 border-b border-border bg-card shrink-0 flex flex-col sm:flex-row sm:items-center justify-between gap-4 z-10 shadow-sm relative">
        <div>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto bg-muted/10 p-4">
          ...
          <div className="max-w-4xl mx-auto">
            {/* Header / Bulk Actions */}
            <div className="flex items-center justify-between mb-6 bg-card border border-border rounded-md p-4 shadow-sm">
              <div>
            ...
            {activeTab === 'addons' ? (
              ...
              <div className="space-y-6">
                {addonGroups.map(group => {
                  ...
                  return (
                    <div key={group.id} className="bg-card border border-border rounded-md overflow-hidden shadow-sm">
                      ...
                      <div className="divide-y divide-border">
                        {groupAddons.map(addon => (
                          <div key={addon.id} className="flex items-center justify-between p-4 hover:bg-muted/20 transition-colors">
                            <div>
                              <p className="font-bold text-[13px] text-foreground">{addon.name}</p>
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              >
                                <div className={`w-4 h-4 bg-white rounded-full absolute transition-all shadow-md ${addon.inStock ? 'left-8' : 'left-1'}`} />
                              </button>
                            </div>
                          </div>
                        ))}
                ...
                {filteredAddons.length === 0 && (
                  <div className="p-5 text-center text-muted-foreground border border-dashed border-border rounded-md">
                ...
                {filteredItems.map(item => (
                  <div key={item.id} className="flex items-center justify-between p-4 hover:bg-muted/20 transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-bold text-[13px] text-foreground">{item.name}</p>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      >
                        <div className={`w-4 h-4 bg-white rounded-full absolute transition-all shadow-md ${item.inStock ? 'left-8' : 'left-1'}`} />
                      </button>
                    </div>
                  </div>
                ))}
