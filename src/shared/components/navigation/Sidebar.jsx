  ...
  const sidebarContent = (isMobile) => (
    <>
      ...
      <div className="h-14 flex items-center justify-between px-6 border-b border-border shrink-0">
        <span className={`text-lg font-bold text-foreground whitespace-nowrap overflow-hidden transition-all duration-300 ${!isMobile && isCollapsed ? 'opacity-0 w-0' : 'opacity-100 w-auto tracking-tight'}`}>
          Restrobopos
        </span>
        {!isMobile && (
          <span className={`text-xl font-bold text-primary transition-all duration-300 ${isCollapsed ? 'opacity-100 block mx-auto' : 'opacity-0 hidden'}`}>
            RB
          </span>
        )}
        {isMobile && (
      ...
      <div className="px-4 pt-4 pb-2">
        <button 
          ...
          className={`w-full flex items-center gap-3 px-3 py-1.5 rounded-md text-[13px] font-medium bg-muted/50 text-muted-foreground hover:bg-muted hover:text-foreground transition-all border border-border/50 hover:border-border ${!isMobile && isCollapsed ? 'justify-center' : ''}`}
        >
            ...
            <div className="space-y-1">
              {group.items.map((item) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-3 py-1.5 rounded-md text-[13px] font-medium transition-all ${
      ...
      <div className="p-4 border-t border-border bg-muted/20 relative" ref={menuRef}>
          ...
          <div className={`absolute ${!isMobile && isCollapsed ? 'left-full ml-4 bottom-0 w-64' : 'bottom-full left-4 right-4 mb-2'} bg-card border border-border rounded-xl shadow-2xl overflow-hidden animate-in fade-in ${!isMobile && isCollapsed ? 'slide-in-from-left-2' : 'slide-in-from-bottom-2'} duration-200 z-[60]`}>
            ...
            <div className="p-2 border-t border-border bg-muted/10">
              <button 
                ...
                className="w-full flex items-center gap-3 px-3 py-1.5 text-[13px] font-medium rounded-md hover:bg-red-50 text-red-600 transition-colors text-left"
              >
        ...
        <button 
          ...
          className={`w-full flex items-center gap-3 transition-all ${!isMobile && isCollapsed ? 'justify-center' : 'px-2 py-1.5 rounded-md bg-card border border-border shadow-sm hover:border-primary/50'}`}
        >
          <div className="w-7 h-7 rounded-md bg-primary text-primary-foreground flex items-center justify-center font-semibold text-[10px] shrink-0">
            JD
          </div>
          {(!isMobile && !isCollapsed || isMobile) && (
            <div className="flex-1 min-w-0 text-left">
              <p className="text-[13px] font-semibold text-foreground truncate">John Doe</p>
    ...
    <>
      ...
      {isMobileOpen && (
        <div className="fixed inset-0 z-[90] md:hidden">
          ...
          />
          {/* Drawer */}
          <div className="absolute inset-y-0 left-0 w-64 bg-card border-r border-border flex flex-col shadow-2xl animate-in slide-in-from-left duration-300 z-10">
            {sidebarContent(true)}
          </div>
        </div>
      )}
