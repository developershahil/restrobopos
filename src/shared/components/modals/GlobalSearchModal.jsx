...
export default function GlobalSearchModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh] sm:pt-[20vh] px-4 bg-background/80 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      <div className="relative bg-card w-full max-w-xl rounded-lg shadow-2xl border border-border overflow-hidden animate-in slide-in-from-top-4 duration-300">
        ...
        <div className="flex items-center px-4 border-b border-border bg-card relative">
          ...
          <input
            ...
            placeholder="Search..."
            className="flex-1 bg-transparent border-none outline-none px-4 py-3 text-[13px] font-medium h-12 text-foreground placeholder:text-muted-foreground"
          />
          <button 
            onClick={onClose}
            ...
            <div className="space-y-1">
              ...
              {filteredResults.map((result, idx) => (
                <button
                  ...
                  className="w-full flex items-center justify-between p-2 rounded-md hover:bg-muted transition-colors group text-left"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-md bg-background border border-border flex items-center justify-center shrink-0">
                      <result.icon className="w-4 h-4 text-muted-foreground group-hover:text-primary transition-colors" />
                    </div>
                    <div>
