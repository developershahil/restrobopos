    ...
    <div className="flex flex-col h-full bg-background absolute inset-0 z-40">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-card border-b border-border shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-foreground">Bulk Edit Items</h2>
            <p className="text-xs text-muted-foreground">Rapidly update pricing, markups, and stock status across your menu.</p>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <button onClick={onClose} className="px-4 py-2 border border-border rounded-lg text-sm font-semibold hover:bg-muted transition-colors">
            Exit Bulk Mode
          </button>
          <button 
            onClick={handleSave}
          ...
          >
            <Save className="w-4 h-4" /> Save {Object.keys(edits).length > 0 ? Object.keys(edits).length : ''} Changes
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 px-6 py-3 border-b border-border bg-muted/20 shrink-0">
        <div className="relative w-64">
          ...
          <input 
            ...
            className="w-full pl-9 pr-3 py-2 bg-background border border-border rounded-lg text-sm outline-none focus:border-primary transition-colors"
          />
        </div>
        <select
          value={filterCat}
          onChange={(e) => setFilterCat(e.target.value)}
          className="px-3 py-2 bg-background border border-border rounded-lg text-sm outline-none focus:border-primary transition-colors cursor-pointer"
        >
      ...
      <div className="flex-1 overflow-auto custom-scrollbar">
        <table className="w-full text-sm text-left">
          <thead className="text-xs text-muted-foreground bg-muted/50 uppercase sticky top-0 z-10 backdrop-blur-sm">
            <tr>
              <th className="px-6 py-3 font-semibold border-b border-border/50">Item Name</th>
              <th className="px-6 py-3 font-semibold border-b border-border/50">Category</th>
              <th className="px-6 py-3 font-semibold border-b border-border/50">Base Price (₹)</th>
              <th className="px-6 py-3 font-semibold border-b border-border/50 text-orange-600">Swiggy Markup (%)</th>
              <th className="px-6 py-3 font-semibold border-b border-border/50 text-red-600">Zomato Markup (%)</th>
              <th className="px-6 py-3 font-semibold border-b border-border/50 text-center">In Stock</th>
            </tr>
          </thead>
          <tbody>
            {filteredItems.map(item => {
              ...
              return (
                <tr key={item.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                  <td className="px-6 py-2.5 font-semibold text-foreground">{item.name}</td>
                  <td className="px-6 py-2.5 text-muted-foreground text-xs">{catName}</td>
