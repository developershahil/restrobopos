...
export default function OffersTab() {
  ...
  return (
    <div className="p-4 max-w-2xl">
      ...
      {offers.length === 0 ? (
        <div className="border border-dashed border-border rounded-xl py-10 text-center text-sm text-muted-foreground">
          No offers yet. Click "Add Offer" to create one.
        </div>
      ) : (
        <div className="space-y-2">
          {offers.map(offer => (
            <div key={offer.id} className={`border border-border rounded-xl p-3 flex items-center gap-3 transition-colors ${offer.active ? 'bg-card' : 'bg-muted/30 opacity-60'}`}>
