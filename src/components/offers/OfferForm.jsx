  ...
  return (
    <div className="space-y-5">
                ...
                {(form.applicability === 'specific_categories' ? form.selectedCategories : form.selectedItems).length > 0 && (
                  <div className="space-y-2 max-h-80 overflow-y-auto">
                    {MOCK_SELECTED_ITEMS.map((item, idx) => (
                      <div key={idx} className="flex items-center gap-3 bg-background border border-border rounded-lg px-4 py-3">
            ...
            <div className="mt-4 space-y-4">
              {DAYS.map(day => (
                <div key={day} className="border border-border rounded-lg p-4">
                  <p className="font-semibold text-sm text-foreground mb-3">{day}</p>
      ...
      <Section title="Terms & Conditions">
        <div>
          ...
          <textarea
            ...
            rows={5}
            className="w-full bg-background border border-border rounded-lg px-4 py-3 text-sm font-medium outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors resize-y"
          />
        </div>
      </Section>
      ...
      <div className="flex items-center justify-end gap-3 pt-2 pb-6">
        <button onClick={onBack} className="px-6 py-2.5 text-sm font-bold text-destructive hover:bg-destructive/10 rounded-lg transition-colors">
          Discard changes
        </button>
